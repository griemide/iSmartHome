  ...
  if ((WiFi.status() == WL_CONNECTED) && (static_cast<uint32_t>(WiFi.localIP()) != 0)) {
    ...
    switch (WiFi.status()) {
      ...
      case WL_CONNECT_FAILED:
         addLog_P(LOG_LEVEL_DEBUG, PSTR("Wifi: STATION_CONNECT_FAIL")); // M. Gries
...
void WIFI_Check(int param)
{
  ...
  default:
    if (_wifiConfigCounter) {
      ...
      if (_wificounter <= 0) {
        addLog_P(LOG_LEVEL_DEBUG_MORE, PSTR("Wifi: Checking connection..."));                           // M. Gries
        _wificounter = WIFI_CHECKSEC;
        WIFI_check_ip();
      }
#ifdef USE_WEBSERVER
...
//  WiFi.setAutoConnect(true);
  if (!strncmp(ESP.getSdkVersion(),"1.5.3",5)) {
    //addLog_P(LOG_LEVEL_DEBUG, "Wifi: Patch issue 2186");                                   //M. Gries
...
//  delay(250);
  ...
  if (dht_expectPulse(LOW) == 0) {
    addLog_P(LOG_LEVEL_DEBUG, PSTR("DHT: Timeout waiting for start signal low pulse"));  // M. Gries
    _lastresult = false;
    return _lastresult;
  }
  if (dht_expectPulse(HIGH) == 0) {
    addLog_P(LOG_LEVEL_DEBUG, PSTR("DHT: Timeout waiting for start signal high pulse"));  // M. Gries
    _lastresult = false;
    return _lastresult;
  }
  ...
  for (int i=0; i<40; ++i) {
    ...
    if ((lowCycles == 0) || (highCycles == 0)) {
      addLog_P(LOG_LEVEL_DEBUG, PSTR("DHT: Timeout waiting for pulse"));                // M. Gries
      _lastresult = false;
      return _lastresult;
    }
    data[i/8] <<= 1;
  ...
  if (data[4] == ((data[0] + data[1] + data[2] + data[3]) & 0xFF)) {
    ...
    addLog_P(LOG_LEVEL_DEBUG, PSTR("DHT: Checksum failure"));                           // M. Gries
    _lastresult = false;
    return _lastresult;
  }
}
